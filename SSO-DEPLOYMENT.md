...
운영 배포: 2026-09-20. 중앙 DB SSO 등록 및 두 Vercel 프로젝트 배포 완료. 기존 회원 2명과 암호화키를 유지했습니다. 비로그인 접근 차단·중앙 클라이언트 인증·중앙 로그인 화면 이동을 운영에서 확인했습니다. 실제 승인 계정으로 Job Star 진입과 중앙 로그인 확인, 통합 로그아웃 후 중앙 접근 차단을 확인했습니다. 실제 Gemini 문서 생성은 외부 API 실패로 결과 생성에 성공하지 못했으며, 호출 실패 표시와 기본 결과 미생성은 확인했습니다. 아래 운영 적용 순서는 이후 재설치·변경 시 참고용입니다.
